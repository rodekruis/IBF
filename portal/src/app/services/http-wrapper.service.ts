...
export type PerformApiServiceRequestParams = { endpoint: string } & Omit<
...
export class HttpWrapperService {
  ...
  private handleError(errorResponse: HttpErrorResponse) {
    ...
    return of(
      new Error($localize`:@@generic-error:Error`, {
        cause: errorResponse,
      }),
    );
  }
  ...
  public async performApiServiceRequest<T>(
    options: PerformApiServiceRequestParams,
  ): Promise<T> {
    return this.performRequest<T>({
      ...options,
      url: `${environment.url_api_service}/${options.endpoint}`,
    });
  }
}
