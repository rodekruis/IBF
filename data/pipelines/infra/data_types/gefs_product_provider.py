...
def download_gefs_product_from_seed_repo(product: str, variant: str) -> list[str]:
    """
    Download one GEFS product's cycle from the seed repo and return the local file paths.

    `product` is both the seed-repo subdirectory and the cache namespace, e.g. "gefs-wind"
    (per-member GRIB2) or "gefs-track" (ATCF track files). `variant` selects the scenario manifest
    `"alert"` for the `--mock 1` over-threshold cycle or `"no-alert"` for the `--mock 0` cycle
    """
    ...
    product_base_url = f"{base_url}{SEED_REPO_TROPICAL_CYCLONE_PATH}{product}"
    manifest_url = f"{product_base_url}/manifest.{variant}.json"
    ...
    cache_dir = get_tropical_cyclone_mock_data_dir(product)
