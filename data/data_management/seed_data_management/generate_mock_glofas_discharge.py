...
Usage:
    ...
    uv run python data_management/seed_data_management/generate_mock_glofas_discharge.py --country ETH --exceed-return-period 20
    uv run python data_management/seed_data_management/generate_mock_glofas_discharge.py --country ETH --alert-lead-times 0,1,2
...
def generate_alert_file(
    ...
    stations: list[dict],
    output_dir: Path,
    alert_lead_times: list[int] | None = None,
    exceed_return_period: float = 5.0,
) -> Path:
    lat_coords, lon_coords, _, _ = compute_country_grid(stations)
    height = len(lat_coords)
    width = len(lon_coords)
    ...
    if alert_lead_times is None:
        alert_lead_times = list(range(NUM_LEAD_TIMES))
    ...
    for station in stations:
        ...
        if station["station_code"] == alert_station["station_code"]:
            target_threshold = _find_threshold_value(station, exceed_return_period)
            alert_value = target_threshold * 1.5
            safe_value = rp_1_5_threshold * 0.5
            for lead_time in range(NUM_LEAD_TIMES):
                if lead_time in alert_lead_times:
    ...
    logger.info(
        ...
        f"exceeding RP{exceed_return_period} threshold={_find_threshold_value(alert_station, exceed_return_period):.2f}, "
        f"alert value={_find_threshold_value(alert_station, exceed_return_period) * 1.5:.2f}, "
        f"lead times={alert_lead_times}"
    )
    return output_path


def _find_threshold_value(station: dict, return_period: float) -> float:
    for t in station["thresholds"]:
        if t["return_period"] == return_period:
            return t["threshold_value"]
    available = [t["return_period"] for t in station["thresholds"]]
    raise ValueError(
        f"Return period {return_period} not found for station {station['station_code']}. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--alert-lead-times",
        help="Comma-separated lead time indices (0-7) that should exceed threshold. "
        "Default: all lead times. Example: '0,1,2' or '3,4,5,6,7'",
    )
    parser.add_argument(
        "--exceed-return-period",
        type=float,
        default=5.0,
        help="Return period threshold to exceed at the alert station (default: 5.0). "
        "Typical values: 1.5, 2, 5, 10, 20, 50",
    )
    ...
    alert_lead_times: list[int] | None = None
    if args.alert_lead_times:
        alert_lead_times = [int(x.strip()) for x in args.alert_lead_times.split(",")]
    ...
    generate_alert_file(
        ...
        stations,
        output_dir,
        alert_lead_times=alert_lead_times,
        exceed_return_period=args.exceed_return_period,
    )
